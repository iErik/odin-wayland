...
foreign wayland {
  ...
  wl_display_add_global    :: proc (
    ...
    interface: ^wl_interface,
    data: rawptr,
    bind: wl_global_bind_func) -> ^wl_global ---
  @(deprecated="")
  ...
  wl_priv_signal_get        :: proc (
    signal: ^wl_priv_signal,
    notify: wl_notify_func_t) -> ^wl_listener ---
  ...
  wl_global_create :: proc (
    ...
    version: i32,
    data: rawptr,
    bind: wl_global_bind_func_t) -> ^wl_global ---
  ...
  wl_global_get_interface :: proc (
    global: ^wl_global) -> ^wl_interface ---
  wl_global_get_name :: proc (
    global: ^wl_global,
    client: ^wl_client) -> u32 ---
  wl_global_get_version :: proc (
    global: ^wl_global) -> u32 ---
  wl_global_get_display :: proc (
    global: ^wl_global) -> ^wl_display ---
  wl_global_get_user_data :: proc (
    wl_global: ^wl_global) -> rawptr ---
  ...
  wl_display_get_client_list :: proc (
    display: ^wl_display) -> ^wl_list ---
  wl_client_get_link :: proc (
    client: ^wl_client) -> ^wl_list ---
  wl_client_from_link :: proc (
    link: ^wl_list) -> ^wl_client ---
  ...
  wl_resource_instance_of :: proc (
    resource: ^resource,
    #by_ptr interface: wl_interface,
    implementation: rawptr) -> i32 ---
  wl_resource_get_class :: proc (
    resource: ^wl_resource) -> string ---
  ...
  wl_shm_buffer_create :: proc (
    ...
    height: i32,
    stride: i32,
    format: u32) -> ^wl_shm_buffer ---
  wl_log_set_handler_server :: proc (
    handler: wl_log_func_t) ---
  wl_display_add_protocol_logger :: proc (
    display: ^wl_display,
    func: wl_protocol_logger_func_t,
    user_data: rawptr) -> ^wl_protocol_logger ---
  ...
  wl_interface_equal :: proc (
    ...
    #by_ptr iface2: wl_interface) -> i32 ---
  ...
  wl_map_lookup :: proc (
    aMap: ^wl_map, i: u32) -> rawptr ---
  wl_map_lookup_flags :: proc (
    aMap: ^wl_map, i: u32) -> u32 ---
  ...
  wl_connection_create :: proc (
    fd: i32,
    max_buffer_size: uint) -> ^wl_connection ---
  wl_connection_destroy :: proc (
    connection: ^wl_connection) -> i32 ---
  ...
  get_next_argument :: proc (
    signature: cstring,
    #by_ptr details: argument_details) -> string ---
  arg_count_for_signature :: proc (
    signature: cstring) -> i32 ---
  wl_message_count_arrays :: proc (
    #by_ptr message: wl_message) -> i32 ---
  ...
  wl_closure_marshal :: proc (
    ...
    opcode: u32,
    #by_ptr args: wl_argument,
    #by_ptr message: wl_message) -> ^wl_closure ---
  ...
  wl_os_socket_peercred :: proc (
    ...
    gid: ^gid_t,
    pid: ^pid_t) -> i32 ---
  wl_os_dupfd_cloexec :: proc (
    fd: i32, minfd: i32) -> i32 ---
  ...
  wl_proxy_create :: proc (
    factory: ^wl_proxy,
    #by_ptr interface: wl_interface) -> ^wl_proxy ---
  wl_proxy_create_wrapper :: proc (
    proxy: rawptr) -> rawptr ---
  wl_proxy_wrapper_destroy :: proc (
    proxy_wrapper: rawptr) ---
  ...
  wl_proxy_add_listener :: proc (
    ...
    implementation: proc (),
    data: rawptr) -> i32 ---
  wl_proxy_get_listener :: proc (
    proxy: ^wl_proxy) -> rawptr ---
  ...
  wl_proxy_set_user_data :: proc (
    proxy: ^wl_proxy,
    user_data: rawptr) ---
  wl_proxy_get_user_data :: proc (
    prox: ^wl_proxy) -> rawptr ---
  ...
  wl_proxy_set_tag :: proc (
    proxy: ^wl_proxy,
    tag: ^cstring) ---
  wl_proxy_get_tag :: proc (
    proxy: ^proxy, tag: ^cstring) ---
  wl_proxy_get_class :: proc (
    proxy: ^wl_proxy) -> string ---
  wl_proxy_get_display :: proc (
    proxy: ^wl_proxy) -> ^wl_display ---
  wl_proxy_set_queue :: proc (
    proxy: ^wl_proxy,
    queue: ^wl_event_queue) ---
  wl_proxy_get_queue :: proc (
    #by_ptr proxy: wl_proxy) -> ^wl_event_queue ---
  wl_event_queue_get_name :: proc (
    #by_ptr queue: wl_event_queue) -> cstring ---
  wl_display_connect :: proc (
    name: cstring) -> ^wl_display ---
  wl_display_connect_to_fd :: proc (
    fd: i32) -> ^wl_display ---
  wl_display_disconnect :: proc (
    display: ^wl_display) ---
  wl_display_get_fd :: proc (
    display: ^wl_display) -> i32 ---
  wl_display_dispatch :: proc (
    display: ^wl_display) -> i32 ---
  wl_display_dispatch_queue :: proc (
    display: ^wl_display,
    queue: ^wl_event_queue) -> i32 ---
  wl_display_dispatch_queue_pending :: proc (
    display: ^wl_display,
    queue: ^wl_event_queue) -> i32 ---
  wl_display_dispatch_pending :: proc (
    display: ^wl_display) -> i32 ---
  wl_display_get_error :: proc (
    display: ^wl_display) -> i32 ---
  ...
  wl_display_roundtrip_queue :: proc (
    display: ^wl_display,
    queue: ^wl_event_queue) -> i32 ---
  wl_display_roundtrip :: proc (
    display: ^wl_display) -> i32 ---
  wl_display_create_queue :: proc (
    display: ^wl_display) -> ^wl_event_queue ---
  wl_display_create_queue_with_name :: proc (
    display: ^wl_display,
    name: cstring) -> ^wl_event_queue ---
  ...
  wl_display_read_events :: proc (
    display: ^wl_display) -> i32 ---
  wl_log_set_handler_client :: proc (
    handler: wl_log_func_t) ---
